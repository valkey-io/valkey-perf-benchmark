...
def create_module_table(conn, module_name: str) -> None:
    """Create module benchmark tracking table if it doesn't exist."""
        ...
        cur.execute(
            ...
            CREATE TABLE IF NOT EXISTS {table} (
                ...
                CONSTRAINT unique_{table}_sha_module_config_arch
                    UNIQUE(sha, module_sha, config, architecture)
            );
            ...
            CREATE INDEX IF NOT EXISTS idx_{table}_config ON {table} USING GIN(config);
...
def determine_module_commits(
    ...
    """Return up to max_pairs 'core_sha:module_sha' pairs not yet benchmarked.

    Ordering: fix newest core SHA, sweep module SHAs newest-to-old.
    Then move to the next core SHA and repeat.
    ...
    Returns:
        List of 'core_sha:module_sha' strings
    """
    # Ensure table exists (mirrors: create_tables in core determine)
    create_module_table(conn, module_name)

    # Clean up incomplete pairs from previous failed runs
    # (mirrors: cleanup_incomplete_commits in core determine)
    cleanup_module_commits(conn, module_name)

    # Get all commits from both git repos (newest first)
    # (mirrors: _git_rev_list(repo, branch) in core determine)
    core_shas = _git_rev_list(repo, branch)
    module_shas = _git_rev_list(module_repo, module_branch)

    # Get completed pairs for exact config match
    # (mirrors: SELECT DISTINCT sha FROM benchmark_commits WHERE status='complete')
    table = _module_table_name(module_name)
    with conn.cursor() as cur:
        if config:
            cur.execute(
    ...
    # Find pairs that need benchmarking
    # (mirrors: iterating all_shas and skipping completed ones in core)
    # Ordering: newest core SHA first, then sweep all module SHAs newest-to-old
    pairs = []
    for core_sha in core_shas:
        for module_sha in module_shas:
            if (core_sha, module_sha) not in completed:
                pairs.append(f"{core_sha}:{module_sha}")
...
def mark_module_commits(
    ...
    """Mark module commit pairs with status.

    Mirrors mark_commits from core, but operates on (core_sha, module_sha) pairs
    instead of single SHAs.

    Args:
        conn: PostgreSQL connection
        repo: Path to valkey (core) git repository
        module_repo: Path to module git repository
        pairs: List of 'core_sha:module_sha' strings
        status: Status to set ('in_progress', 'complete')
        architecture: Architecture
        module_name: Module name (determines table)
        config: Config content
    """
    # Ensure table exists (mirrors: create_tables in core mark)
    create_module_table(conn, module_name)
    table = _module_table_name(module_name)
    ...
    with conn.cursor() as cur:
        for pair in pairs:
            ...
            # Get git commit timestamps for both SHAs
            # (mirrors: ts = _git_commit_time(repo, sha) in core mark)
            ts = _git_commit_time(repo, core_sha)
            module_ts = _git_commit_time(module_repo, module_sha)

            # Insert or update
            # (mirrors: INSERT ... ON CONFLICT DO UPDATE in core mark)
            cur.execute(
                f"""
                INSERT INTO {table} (sha, module_sha, timestamp, module_timestamp,
                                     status, config, architecture)
                VALUES (%s, %s, %s, %s, %s, %s, %s)
                ON CONFLICT (sha, module_sha, config, architecture)
                DO UPDATE SET
                    status = EXCLUDED.status,
                    updated_at = NOW()
            """,
                (
                    ...
                    module_ts,
                    status,
                    Json(config) if config else Json({}),
                    architecture,
                ),
            )

            # Format config for display (mirrors: config_display in core mark)
            config_display = ""
            if config:
                if isinstance(config, list) and len(config) > 0:
                    first_cfg = config[0]
            ...
            print(
                f"Marked {core_sha[:8]}:{module_sha[:8]} ({module_name} on {architecture}) as {status} with timestamps {ts}/{module_ts}{config_display}",
                file=sys.stderr,
            )
...
def cleanup_module_commits(conn, module_name: str) -> int:
    """Remove all 'in_progress' entries from module tracking table.

    Mirrors cleanup_incomplete_commits from core, but operates on
    the module-specific table.
    ...
    if count > 0:
        print(
            f"Cleaned up {count} incomplete entries from {table}", file=sys.stderr
        )
