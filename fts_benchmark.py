...
class FTSBenchmarkRunner(ClientRunner):
    ...
    def __init__(self, *args, **kwargs):
        ...
        server_type = kwargs.pop("server_type", "auto")  # auto, valkey, or redis

        super().__init__(*args, **kwargs)
        self.current_index = None
        self.profiler = PerformanceProfiler(self.results_dir, profiling_enabled)

        # Determine target process name for CPU monitoring
        if server_type == "auto":
            # Auto-detect: search for both valkey and redis
            target_process = None  # Will search for both in cpu_monitor
        elif server_type == "valkey":
            target_process = "valkey-server"
        elif server_type == "redis":
            target_process = "redis-server"
        else:
            target_process = server_type  # Allow custom process name

        self.cpu_monitor = CPUMonitor(
            target_process=target_process if target_process else "redis-server"
        )
        logging.info(
            f"CPU monitoring configured for: {server_type} (target_process={target_process or 'auto-detect'})"
        )
    ...
    def _run_backfill_test_group(self, test_group: Dict[str, Any]) -> None:
        ...
        for search_test in test_group["searches"]:
            if search_test.get("concurrent_with_backfill"):
                ...
                search_desc = search_test["description"]
                logging.info(
                    f"Phase: {group_id}{sub_id} ({search_desc}) - concurrent with backfill"
                )
        ...
        try:
            with self._client_context() as client:
                ...
                for i in range(0, len(info), 2):
                    key = (
                        info[i].decode() if isinstance(info[i], bytes) else str(info[i])
                    )
                    value = info[i + 1]
        ...
        while time.time() - start_time < timeout:
            try:
                with self._client_context() as client:
                    ...
                    logging.info(
                        f"Backfill in progress: {backfill_pct} (state: {state})"
                    )
                    time.sleep(5)
            except Exception as e:
    ...
    def _create_fts_index(self, ingestion_config: Dict[str, Any]) -> None:
        ...
        # Extract index parameters from config with defaults
        index_name = ingestion_config.get("index_name", "rd0")
        index_type = ingestion_config.get("index_type", "HASH")
        prefix_count = ingestion_config.get("prefix_count", 1)
        prefix_value = ingestion_config.get("prefix_value", "rd0-")
        ...
        # Add config options before SCHEMA
        ...
        elif config == "NOSTEM+WITHOFFSETS":
            create_cmd.append("WITHOFFSETS")

        create_cmd.append("SCHEMA")
        schema_fields = []

        # Parse schema from command to determine fields
        command = ingestion_config.get("command", "")
        if "field1" in command and "field49" in command:
            # Multi-field test: create schema for field1-field49 + title
            for i in range(1, 50):
                schema_fields.extend([f"field{i}", "TEXT"])
            schema_fields.extend(["title", "TEXT"])
            logging.info(f"Creating index with 49 data fields + title (50 total)")
        else:
            # Default schema
            schema_fields.extend(["title", "TEXT", "body", "TEXT", "url", "TEXT"])

        # Apply NOSTEM per-field if requested
        if config in ["NOSTEM", "NOSTEM+WITHOFFSETS"]:
            schema_with_nostem = []
            for i in range(0, len(schema_fields), 2):
                schema_with_nostem.extend(
                    [schema_fields[i], schema_fields[i + 1], "NOSTEM"]
                )
            schema_fields = schema_with_nostem
            logging.info(f"Applied NOSTEM to {len(schema_fields)//3} fields")

        create_cmd.extend(schema_fields)
        logging.info(f"Creating FTS index '{index_name}' with config: {config}")
        self._flush_database()
    ...
    def _flush_database(self) -> None:
        ...
        logging.info(
            "Flushing database for clean state (may take 30+ seconds for large datasets)"
        )
        try:
    ...
    def _run_ingestion_phase(
        ...
        self.cpu_monitor.start_monitoring(f"ingestion_{test_id}")
        ...
        cpu_stats = self.cpu_monitor.stop_monitoring(f"ingestion_{test_id}")
        ...
        if proc:
            ...
            if metrics:
                ...
                if "field_range" in ingestion_config:
                    metrics["field_count"] = len(
                        ingestion_config["field_range"].split("-")
                    )
                # Add CPU monitoring stats
                metrics.update(cpu_stats)
                self.all_metrics.append(metrics)

    def _run_search_phase(self, search_config: Dict[str, Any], test_id: str) -> None:
        """Run search phase with dataset support."""
        dataset = search_config.get("dataset", None)  # Optional dataset
        ...
        # Start CPU monitoring
        self.cpu_monitor.start_monitoring(f"search_{test_id}")
        ...
        # Stop CPU monitoring and get stats
        cpu_stats = self.cpu_monitor.stop_monitoring(f"search_{test_id}")
...
def run_fts_benchmarks(
    ...
    profiling = profiling_enabled or fts_config.get("profiling", {}).get(
        "enabled", False
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--target-ip", default="127.0.0.1", help="Target Valkey server IP"
    )
    parser.add_argument(
        "--config", default="configs/fts-search-configs.json", help="FTS config file"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--valkey-benchmark-path", help="Path to valkey-benchmark executable"
    )
